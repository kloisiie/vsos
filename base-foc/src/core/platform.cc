...
static void _core_pager_loop()
{
	...
	while (true) {
		...
		/*l4_umword_t pointer=l4_utcb_mr()->mr[0];
		l4_umword_t senderID=l4_utcb_id()->senderID;
		l4_umword_t receiverID=l4_utcb_id()->receiverID;*/
		if (pfa < (l4_umword_t)L4_PAGESIZE) {

			/* NULL pointer access */
			PERR("Possible null pointer %s at %lx IP %lx"/*,pfa %lx,id:%d :%d"*/,
			     rw ? "WRITE" : "READ", pfa, ip/*,pointer,senderID,receiverID*/);
