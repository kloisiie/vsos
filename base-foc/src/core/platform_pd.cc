...
Platform_pd::Platform_pd(const char *str)
: _task(true, TASK_CAP)
{
	static int id=0;
	id++;	
	for (unsigned i = 0; i < THREAD_MAX; i++)
		_threads[i] = (Platform_thread*) 0;
	char name[50];
	Arg_string::find_arg(str,"name").string(name,50,str);
	Monitor_Connection h;
	int taskID=h.SetTaskID(name, id);
	if(taskID<0) taskID=id;
	PERR("task name %s, %d",name,taskID);
	...
	l4_msgtag_t tag = l4_factory_create_task(L4_BASE_FACTORY_CAP,
	                                         _task.local.dst(), utcb_area,taskID);
