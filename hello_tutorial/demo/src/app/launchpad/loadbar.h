...
class Loadbar_event_handler : public Event_handler
{
	...
	public:
		...
		//empty constructor, for simulate buttom click in order to restart a child process.
		//Loadbar_event_handler(){ }
		//added by malele end
		...
		void handle(Event &ev)
		{
			...
			if (ev.type == Event::PRESS || ev.type == Event::MOTION)
				if (_listener && key_cnt > 0)
				{
					...
					//added by malele 20130917 begin
					// for test the call path
					//event fired when point to the loadbar(memory bar). and pressed or motioned on a bar.
					//PDBG("Smeller:Loadbar_event_handler handle the event\n");
					//added by malele 20130917 end
				}
