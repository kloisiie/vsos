...
class Launchpad
{
	...
	public:
		...
		virtual void add_launcher(const char *filename,const char *appname,
		                          unsigned long default_quota,
		                          Genode::Dataspace_capability config_ds, int groupNum) { }
	...
	public:
		...
		:
			Init::Traditional_child_policy(name, server, parent_services,
			                               ...
			                               binary_ds, 0, 0, 0, parent_entrypoint),_launchpad(launchpad),_config_ds(config_ds)
		{ }
		...
		{
			PDBG("hellos hellos helloshelloshellos%d", exit_value);
		//	char _name[16];
		//	Genode::memcpy(_name,this->name(),Genode::strlen(this->name()));
			_launchpad->killR_child(this->name());
		}
};
	...
	public:
			...
			:
				...
				_policy(name, &_server, parent_services, child_services,
				        config_ds, elf_ds, &_entrypoint, launchpad),
				_child(elf_ds, ram, cpu, rm, &_entrypoint, &_policy) {
				_entrypoint.activate(); 
				//PERR("unique name %s launchpad_child name %s",name,_policy.name());
				}
			...
			/*void destruct_child(){ 
				PERR("destruct launchpad_child");
				_child.~Child();				
				//_child.destruct_child();
				PERR("destruct child successfully");
