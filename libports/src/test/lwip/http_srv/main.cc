...
const static char hello[] ="hello,client/n";

const static char server_intr[] ="this is a http server/n";
...
void http_server_serve(int conn) {
	...
	if (buflen > 0) {
		PLOG("Packet content:%s",buf);
		/* Is this an HTTP GET command? (only check the first 5 chars, since
			PLOG("Will send response");

			/* Send http header */
			PLOG("sending response:%s,%s",hello,server_intr);
			lwip_send(conn, hello, Genode::strlen(hello), 0);

			/* Send our HTML page */
			lwip_send(conn, server_intr, Genode::strlen(server_intr), 0);
	}
}
