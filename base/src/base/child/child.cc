			...
			public:
				...
				: _ack(false), _quantum(quantum), _from(from), _to(to)
				{
					if (_from.valid() && _to.valid() &&
						...
						PERR("not enough quota for a donation of %zd bytes", quantum);
...
{
//	PWRN("child annouce %s",name.string());
...
{
//	PWRN("child session %s",name.string());
	if (!name.is_valid_string() || !args.is_valid_string()) throw Unavailable();

//add by wyq
	if(!Genode::strcmp(name.base(),"Confidential"))
	{
		PERR("Confidential session");
		static Monitor_Connection h;
		if(h.cap_judge(name.base())==-1)
		{
			PERR("Confidential session deny");
...
void Child::upgrade(Session_capability to_session, Parent::Upgrade_args const &args)
{
	...
	if (!targeted_service) {
		PERR("could not lookup service for session upgrade");
		return;
	}

	if (!args.is_valid_string()) {
		PERR("no valid session-upgrade arguments");
		return;
	}
...
void Child::close(Session_capability session_cap)
{
	...
	//PERR("close session begin:%d",session_cap.local_name());	
	...
	//PERR("session lookup");
	if (!s) {
		PERR("no session structure found");
		return;
	}
