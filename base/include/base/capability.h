	...
	class Capability : public Untyped_capability
	{
		...
		public:
			     ...
			     typename Arg<IF, 4>::Type v5) const
			{
				...
				_call<IF>(args, ret.value());
				//PERR("GET NAME %s",IF::get_name());
/*				if(!Genode::strcmp(IF::get_name(),"GetTaskStrategyByname"))
				{
					PERR("Rpc_GetTaskStrategyByname call");
					Rpc_out_buffer<26> *m_temp = &args._2._2._2.get();
					PERR("BLP SIZE :%d",(*m_temp).size());
					for(int i=0;i<(*m_temp).size();i++)
					{
						PERR("BLP %d : %d",i,(*m_temp).getball()[i]);
					}
				}
*/
				return ret.value();
