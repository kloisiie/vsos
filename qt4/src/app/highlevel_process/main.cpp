...
namespace Highlevel{
	...
	class Root_component : public Genode::Root_component<Session_component>
	{
		protected:
			...
			Highlevel::Session_component *_create_session(const char *args)
			{
				PDBG("creating highlevel session.");
...
int main(int argc,char* argv[]) {
	...
	static Rpc_entrypoint ep(&cap, STACK_SIZE, "highlevel_ep");

	static Highlevel::Root_component lowlevel_root(&ep, &sliced_heap);
	...
	dialog->setWindowIcon(QIcon(":/appicon/highlevel_process.png"));

	dialog->move((QApplication::desktop()->width()-dialog->width())/2 + 20,300);
	dialog->show();
	log.start();
