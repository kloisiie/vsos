...
namespace Lowlevel{
	...
	class Root_component : public Genode::Root_component<Session_component>
	{
		protected:
			...
			Lowlevel::Session_component *_create_session(const char *args)
			{
				PDBG("creating lowlevel session.");
...
int main(int argc,char* argv[]) {
	...
	static Rpc_entrypoint ep(&cap, STACK_SIZE, "lowlevel_ep");

	static Lowlevel::Root_component lowlevel_root(&ep, &sliced_heap);
	...
	dialog->setWindowIcon(QIcon(":/appicon/lowlevel_process.png"));

	dialog->move((QApplication::desktop()->width()-dialog->width())/2 + 20,0);
	dialog->show();
	log.start();
