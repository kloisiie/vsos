...
{	
	...
	lineEdit_pass->installEventFilter(this);
	lineEdit_pass2->installEventFilter(this);
...
void LoginDialog::loginClicked() {
	...
	if(i<num){
		if((comboBox_blp->currentText()).toInt()>=get_blp/10&&(comboBox_blp->currentText()).toInt()<=get_blp%10){
			...
			this->hide();
			h.AddAuditInfo(lineEdit_user->text().toLatin1().data(),"user login",1,(comboBox_blp->currentText()).toInt());
			loginfinished=false;
			...
			while(!loginfinished){
				...
				if(loginfinished){
					h.AddAuditInfo(name,"sak pressed",1,(comboBox_blp->currentText()).toInt());
			...
			h.AddAuditInfo(lineEdit_user->text().toLatin1().data(),"user login",0,(comboBox_blp->currentText()).toInt());
...
void LoginDialog::changeClicked() {
	...
	if(j<num){
		...
		if(length<8){
			...
			h.AddAuditInfo(lineEdit_user->text().toLatin1().data(),"change user password",0,(comboBox_blp->currentText()).toInt());
			lineEdit_pass2->clear();
			changeButton->setEnabled(false);
...
bool LoginDialog:: eventFilter(QObject *watched,QEvent *e){
	if((watched==changeButton||watched==loginButton||watched==lineEdit_pass||watched==lineEdit_pass2)){
		int type=0;
			...
			if(k->key()==Qt::Key_Enter||k->key()==Qt::Key_Return){
				if(type==2){
					if(isEnter){
						isActive=true;
						isEnter=false;
						if(watched==changeButton||watched==lineEdit_pass2)
							changeClicked();
						else
							loginClicked();						
					}								
