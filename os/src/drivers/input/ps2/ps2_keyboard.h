...
class Ps2_keyboard : public Input_driver
{
	...
	public:
		...
		void handle_event()
		{
			...
			/* begin -- wangs's code */
			/* control the sequence of pressing keys. */
			if(SAK_1 == -1 && _key_state[24] == true && _key_state[31] == false && _key_state[6] == false)
				SAK_1 = 24;
			if(_key_state[24] == true && _key_state[31] == true && _key_state[6] == false)
				SAK_2 = 55;
			if(_key_state[24] == true && _key_state[31] == true && _key_state[6] == true && key_code == 6)
				SAK_3 = 61;

			/* count the number of pressed keys. */
			for(pressed = 0; pressed < Input::KEY_MAX + 1; pressed++)
				if(_key_state[pressed] == true) key_cnt++;

			/* judge the situation of pressed SAK. */
			if ((key_cnt == 3) && (SAK_1 == 24) && (SAK_2 == 55) && (SAK_3 == 61)){
				SAKserver::Connection s;
				SAK_1 = -1; SAK_2 = -1; SAK_3 = -1;
				PWRN("\nSecure attention key (SAK) was pressed.\n");
				s.say_hello();

				int foo = s.add(2, 5);
				PWRN("Added 2 + 5 = %d", foo);
			}
			/* end -- wangs's code */
