...
static uint64_t get_rtc_time(int& year,int& mon,int& day,int& hour,int& min,int& sec,bool bigInt);
namespace Rtc {

	class Session_component : public Genode::Rpc_object<Session>
	{
		public:
			uint64_t get_current_time(int& year,int& mon,int& day,int& hour,int& min,int& sec,bool bigInt)
			{
				uint64_t ret =get_rtc_time(year,mon,day,hour,min,sec,bigInt);
				return ret;
			}
...
static uint64_t get_rtc_time(int& year,int& mon,int& day,int& hour,int& min,int& sec,bool bigInt)
{
	...
	if(hour>=24){
		...
		mon_day[1]=is_leap_year(year)?29:28;
		if(mon_day[mon]<day){
			mon+=1;
			day=1;
			if(mon>12){
